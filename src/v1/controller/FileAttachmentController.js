  ...
  try {
    ...
    if (req.file) {
      imageUrl = await uploadToBackblaze(
        ...
        req.file.mimetype,
        `documents/${req.body.related_entity_type}`
      );
    }
    let userData = {
      ...req.body,
      createdby: req.user.id,
      createdby_name: req.user.full_name || null,
      createdby_email: req.user.email || null,
      file: imageUrl,
    };
    const attachmentData = await AttachmentService.createAttachment(
      userData,
      req.user
    );
    res
      .status(201)
      .success("File attachment created successfully", attachmentData);
  } catch (error) {
    next(error);
  }
};
...
const getCallTypeById = async (req, res, next) => {
  try {
    const attachmentData = await AttachmentService.findAttachmentById(
      req.params.id
    );
    if (!attachmentData)
...
const updateAttachment = async (req, res, next) => {
  try {
    ...
    let userData = {
      ...
      updatedby_name: req.user.full_name || null,
      updatedby_email: req.user.email || null,
      file: imageUrl,
    };
    const attachmentData = await AttachmentService.updateAttachment(
      req.params.id,
      userData,
      req.user
    );
    res
      .status(200)
      .success("File attachment updated successfully", attachmentData);
    if (req.file) {
