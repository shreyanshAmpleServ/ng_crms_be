...
const updateOrder = async (req, res, next) => {
  try {
    ...
    const product = await orderService.updateOrder(
      id,
      orderDAta,
      parsedOrderItemsData
    );
...
const getAllOrder = async (req, res, next) => {
  try {
    ...
    const products = await orderService.getAllOrder(
      search,
      Number(page),
      Number(size),
...
const syncOrderToInvoice = async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    if (!id) {
      return res
        .status(400)
        .json({ success: false, message: "Missing orderId in request body." });
    }

    const invoice = await orderService.syncOrderToInvoice(Number(id));
    res.status(200).success(null, invoice);
  } catch (error) {
    next(error);
  }
};
